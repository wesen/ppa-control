	...
	return layers.NewParameterLayer(
		...
		layers.WithParameterDefinitions(
			parameters.NewParameterDefinition(
				...
				parameters.WithShortFlag("a"), // Changed from "u" to "a" for legacy compatibility
			),
			parameters.NewParameterDefinition(
				"server",
			...
			parameters.NewParameterDefinition(
				...
				parameters.WithShortFlag("p"), // Changed from "P" to "p" for legacy compatibility
			),
			parameters.NewParameterDefinition(
				"interface",
				parameters.ParameterTypeString,
...
// NewUDPBroadcastCommand creates a new UDP broadcast command with glazed support
func NewUDPBroadcastCommand() (*UDPBroadcastGlazedCommand, error) {
	...
	// Create minimal layers for UDP broadcast (no PPA connection settings)
	...
	// Only include UDP broadcast, logging, and glazed layers - no PPA layer
	allLayers := []layers.ParameterLayer{udpBroadcastLayer, loggingLayer, glazedLayer}
